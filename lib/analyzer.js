...
export function scoreOpportunity({
  ...
  // v4: Structured score breakdown (transparent scoring)
  const scoreBreakdown = {
    anchorRelevance:    Math.round(Math.max(0, Math.min(3, raw - Math.max(0, raw - 3))) * 10) / 10,
    keywordOverlap:     Math.round(Math.max(0, Math.min(3, (sharedKeywords?.length || 0) * 0.5)) * 10) / 10,
    paragraphQuality:   wc >= 80 ? 2 : wc >= 40 ? 1.5 : wc >= 20 ? 1 : 0,
    strategicValue:     (target.isOrphan ? 2 : (target.inboundCount || 0) < 2 ? 1 : 0) + (['product','commercial','service'].includes(target.pageType) ? 1 : 0),
    positionBonus:      paraPosition <= 0.25 ? 1 : paraPosition <= 0.50 ? 0.5 : 0,
    penalties:          GENERIC_ANCHORS.has((anchor || '').toLowerCase()) ? -3 : anchor?.length < 4 ? -2 : 0,
    total:              score,
  };

  return { score, confidence, reasons, warnings, scoreBreakdown };
}
  ...
  for (const source of pages) {
    ...
    for (const target of pages) {
      ...
      source.paragraphs.forEach((para, paraIdx) => {
        ...
        const { score, confidence, reasons, warnings, scoreBreakdown } = scoreOpportunity({
          anchor,
          paragraph: para,
          source,
          target,
        ...
        if (score > bestScore) {
          bestScore  = score;
          bestResult = { para, anchor, score, confidence, reasons, warnings, scoreBreakdown, paraPosition, paraIdx };
        }
      });
      ...
      const {
        para, anchor, score, confidence, reasons, warnings, scoreBreakdown, paraPosition, paraIdx,
      } = bestResult;
      ...
      // v4: Body-content proof + validation proof
      ...
      const validationProof =
        `✓ Valid main-body paragraph from "${source.extractionMethod}". ` +
        `Anchor "${anchor}" exists naturally in visible body content. ` +
        `Source does not already link to target. sourceType: main-body.`;
      ...
      opps.push({
        ...
        reason:            reasons.join('; '),
        bodyContentReason,
        validationProof,   // v4: machine-readable proof of source validity
        scoreBreakdown,    // v4: transparent score components
        sourceType:        'main-body',  // v4: guarantee — never thin-content-fallback
        warnings,
